...
# Config values
$Script:CfgNatsServers = ""
$Script:CfgGraphqlPort = ""
$Script:CfgWebPort = ""
$Script:CfgMqttBroker = ""
$Script:CfgMqttClient = ""
$Script:CfgMqttGroup = ""
$Script:CfgMqttNode = ""
$Script:CfgMqttUser = ""
$Script:CfgMqttPass = ""
...
function Test-ModuleSelected {
    ...
    return $Script:SelectedModules -contains $ModName
}

function Get-ModuleType {
    param([string]$ModName)
    if ($ModName -eq "tentacle-opcua") { return "go" }
    return "deno"
}

function Get-ServiceEntrypoint {
    param([string]$ModName)
    if ($ModName -eq "tentacle-web") { return "build\index.js" }
    return "main.ts"
}
...
function Set-Configuration {
    ...
    # ── Core ──────────────────────────────────────────────────────────────────
    $Script:CfgNatsServers = Read-Host "  NATS server address [nats://localhost:4222]"
    if ([string]::IsNullOrWhiteSpace($Script:CfgNatsServers)) { $Script:CfgNatsServers = "nats://localhost:4222" }

    $Script:CfgGraphqlPort = Read-Host "  GraphQL API port [4000]"
    if ([string]::IsNullOrWhiteSpace($Script:CfgGraphqlPort)) { $Script:CfgGraphqlPort = "4000" }

    $Script:CfgWebPort = Read-Host "  Web dashboard port [3012]"
    if ([string]::IsNullOrWhiteSpace($Script:CfgWebPort)) { $Script:CfgWebPort = "3012" }

    $graphqlUrl = "http://localhost:$($Script:CfgGraphqlPort)/graphql"
    if ($Script:DeployMethod -eq "docker") {
        $graphqlUrl = "http://graphql:$($Script:CfgGraphqlPort)/graphql"
    }

    # ── MQTT (only if selected) ────────────────────────────────────────────────
    if (Test-ModuleSelected "tentacle-mqtt") {
        Write-Host ""
        Write-Host "  MQTT / Sparkplug B Configuration" -ForegroundColor White
        Write-Host ""

        $Script:CfgMqttBroker = Read-Host "  Broker URL [mqtt://localhost:1883]"
        if ([string]::IsNullOrWhiteSpace($Script:CfgMqttBroker)) { $Script:CfgMqttBroker = "mqtt://localhost:1883" }

        $Script:CfgMqttGroup = Read-Host "  Group ID [TentacleGroup]"
        if ([string]::IsNullOrWhiteSpace($Script:CfgMqttGroup)) { $Script:CfgMqttGroup = "TentacleGroup" }

        $Script:CfgMqttNode = Read-Host "  Edge node ID [EdgeNode1]"
        if ([string]::IsNullOrWhiteSpace($Script:CfgMqttNode)) { $Script:CfgMqttNode = "EdgeNode1" }

        $Script:CfgMqttClient = Read-Host "  Client ID [tentacle-mqtt]"
        if ([string]::IsNullOrWhiteSpace($Script:CfgMqttClient)) { $Script:CfgMqttClient = "tentacle-mqtt" }

        $Script:CfgMqttUser = Read-Host "  Username (leave blank if none)"
        if (-not [string]::IsNullOrWhiteSpace($Script:CfgMqttUser)) {
            $secPass = Read-Host "  Password" -AsSecureString
            $Script:CfgMqttPass = [Runtime.InteropServices.Marshal]::PtrToStringAuto(
    ...
    if (Test-ModuleSelected "tentacle-mqtt") {
        $envLines += @(
            ...
            "MQTT_BROKER_URL=$($Script:CfgMqttBroker)",
            "MQTT_CLIENT_ID=$($Script:CfgMqttClient)",
            "MQTT_GROUP_ID=$($Script:CfgMqttGroup)",
            "MQTT_EDGE_NODE=$($Script:CfgMqttNode)"
        )
        if (-not [string]::IsNullOrWhiteSpace($Script:CfgMqttUser)) {
            $envLines += "MQTT_USERNAME=$($Script:CfgMqttUser)"
            $envLines += "MQTT_PASSWORD=$($Script:CfgMqttPass)"
        }
    }
...
function Install-Files {
    ...
    # Find the extracted top-level dir (tentacle-v{version}-{platform}/)
    $extractedRoot = Get-ChildItem -Path $tmpDir -Directory | Select-Object -First 1

    # ── Deno runtime ──────────────────────────────────────────────────────────
    $denoSrc = Join-Path $extractedRoot.FullName "bin\deno.exe"
    if (Test-Path $denoSrc) {
        Copy-Item -Path $denoSrc -Destination (Join-Path $binDir "deno.exe") -Force
        Write-Info "  Installed deno runtime"
    }

    # ── NATS server ──────────────────────────────────────────────────────────
    $natsSrc = Join-Path $extractedRoot.FullName "bin\nats-server.exe"
    if (Test-Path $natsSrc) {
        Copy-Item -Path $natsSrc -Destination (Join-Path $binDir "nats-server.exe") -Force
        Write-Info "  Installed nats-server"
    }

    # ── Shared Deno cache + nats-schema ──────────────────────────────────────
    $denoCache = Join-Path $extractedRoot.FullName "services\deno-cache"
    if (Test-Path $denoCache) {
        Copy-Item -Path $denoCache -Destination (Join-Path $ServicesDir "deno-cache") -Recurse -Force
        Write-Info "  Installed Deno dependency cache"
    }
    $natsSchema = Join-Path $extractedRoot.FullName "services\tentacle-nats-schema"
    if (Test-Path $natsSchema) {
        Copy-Item -Path $natsSchema -Destination (Join-Path $ServicesDir "tentacle-nats-schema") -Recurse -Force
        Write-Info "  Installed tentacle-nats-schema"
    }

    # ── Per-module installation ───────────────────────────────────────────────
    foreach ($mod in $Script:SelectedModules) {
        $modType = Get-ModuleType $mod
        if ($modType -eq "go") {
            $src = Join-Path $extractedRoot.FullName "bin\$mod.exe"
            if (Test-Path $src) {
                Copy-Item -Path $src -Destination (Join-Path $binDir "$mod.exe") -Force
                Write-Info "  Installed $mod (binary)"
            }
            else { Write-Warn "  Binary not found: $mod.exe" }
        }
        else {
            $src = Join-Path $extractedRoot.FullName "services\$mod"
            if (Test-Path $src) {
                Copy-Item -Path $src -Destination (Join-Path $ServicesDir $mod) -Recurse -Force
                Write-Info "  Installed $mod (source)"
            }
            else { Write-Warn "  Service source not found: $mod" }
        }
    }
...
function Install-WindowsServices {
    ...
    foreach ($mod in $Script:SelectedModules) {
        ...
        if ($modType -eq "go") {
            ...
            $ep = Get-ServiceEntrypoint $mod
            $svcDir = Join-Path $ServicesDir $mod
            & nssm install $svcName "$denoExe" "run --allow-all $ep"
...
function Install-BinaryOnly {
    ...
    foreach ($mod in $Script:SelectedModules) {
        ...
        if ($modType -eq "go") {
            ...
            $ep = Get-ServiceEntrypoint $mod
            $svcDir = Join-Path $ServicesDir $mod
            $startLines += "start `"$mod`" /B `"$binDir\deno.exe`" run --allow-all `"$svcDir\$ep`""
        }
    ...
    # For deno services, killing deno.exe would stop all; kill by window title instead
    foreach ($mod in $Script:SelectedModules) {
        $modType = Get-ModuleType $mod
        if ($modType -eq "go") {
            ...
            $stopLines += "taskkill /F /FI `"WINDOWTITLE eq $mod`" >nul 2>&1"
        }
    }
    ...
    foreach ($mod in $Script:SelectedModules) {
        ...
        if ($modType -eq "go") {
            ...
            $ep = Get-ServiceEntrypoint $mod
            $svcDir = Join-Path $ServicesDir $mod
            $ps1Lines += '$procs += Start-Process -FilePath "' + $binDir + '\deno.exe" -ArgumentList "run","--allow-all","' + "$svcDir\$ep" + '" -WorkingDirectory "' + $svcDir + '" -PassThru -WindowStyle Hidden'
        }
...
function Show-Summary {
    ...
    Write-Host ""
    Write-Host "  Upgrade Deno runtime:  $InstallDir\bin\deno.exe upgrade"
    Write-Host ""
